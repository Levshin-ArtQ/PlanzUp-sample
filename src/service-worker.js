...
self.addEventListener('install', function(event) {
  event.waitUntil(
    caches.open('settings').then(function(cache) {
      return cache.addAll([
        '/settings.json',
      ]);
    })
...
self.addEventListener('message', function(event) {
  if (event.data.action === 'saveSettings') {
    caches.open('settings.json').then(function(cache) {
      return cache.put(event.data.url, new Response(JSON.stringify(event.data.settings)));
    });
  }
});
